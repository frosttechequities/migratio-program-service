...
export const getTokenFromLocalStorage = () => {
  try {
    ...
    if (token) {
      // For mock tokens, don't try to parse them
      if (token.startsWith('mock-')) {
        console.log('[authUtils] Mock token found, returning without parsing');
        return token;
      }
      ...
      try {
        ...
        console.error('[authUtils] Error parsing token, but returning it anyway', parseError);
        return token; // Return the token even if parsing fails
      }
    }
    ...
    if (supabaseSession) {
      try {
        ...
        if (parsedSession?.access_token) {
          // For mock tokens, don't try to parse them
          if (parsedSession.access_token.startsWith('mock-')) {
            console.log('[authUtils] Mock Supabase token found, returning without parsing');
          ...
          try {
            ...
            console.error('[authUtils] Error parsing Supabase token, but returning it anyway', parseError);
            localStorage.setItem('token', parsedSession.access_token);
            return parsedSession.access_token; // Return the token even if parsing fails
          }
        }
      } catch (parseError) {
      ...
      try {
        ...
        if (user?.token) {
          ...
          if (user.token.startsWith('mock-')) {
            console.log('[authUtils] Mock user token found, returning without parsing');
          ...
          try {
            ...
            console.error('[authUtils] Error parsing user token, but returning it anyway', parseError);
...
export const parseJwt = (token) => {
  try {
    ...
    if (token.startsWith('mock-')) {
      console.log('[authUtils] Mock token detected, returning mock payload');
      return {
        sub: 'mock-user-id',
    ...
    console.error('[authUtils] Error parsing JWT token, returning mock payload', e);
    // Return a mock payload instead of throwing
    return {
      sub: 'mock-user-id',
      email: 'mock@example.com',
      name: 'Mock User',
